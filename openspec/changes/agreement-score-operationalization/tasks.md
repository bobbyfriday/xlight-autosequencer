...
- [ ] 5.4 Add a frontend unit / Vitest test (file: under `src/review/frontend/tests/` matching the existing pattern) that renders the section list with mixed `low_confidence` + `chorus_ssm_supported` values and asserts the indicator behavior. *Deferred: the existing Vitest tests don't cover the Analyze section-list rendering; adding the harness is more invasive than this change should be. The TypeScript types and rendering logic are validated by `npx tsc --noEmit` and the e2e flow.*
